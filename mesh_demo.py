...
def make_animation(source_video, driving_video, driving_mesh_video, source_mesh, driving_mesh, generator, relative=True, adapt_movement_scale=True, cpu=False):
    with torch.no_grad():
        predictions = []
        ...
        driving_mesh_video = torch.tensor(np.array(driving_mesh_video)[np.newaxis].astype(np.float32))
        print(type(driving_mesh_video))
        print(type(source_mesh))
        print(type(driving_mesh))
        for frame_idx in tqdm(range(driving.shape[2])):
            driving_frame = driving[:, :, frame_idx]
            driving_mesh_image = driving_mesh_video[:, :, frame_idx]
...
if __name__ == "__main__":
    ...
    predictions = make_animation(dataset['video'], dataset['driving_video'], dataset['driving_mesh_img'], dataset['mesh'], dataset['driving_mesh'], generator, relative=opt.relative, adapt_movement_scale=opt.adapt_scale, cpu=opt.cpu)
